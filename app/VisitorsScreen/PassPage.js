...
const PassPage = ({ nightMode, passData, loading, onRefresh }) => {
  ...
  return (
    <View style={[styles.container, { backgroundColor: currentTheme.backgroundColor }]}>
      ...
      {/* Add Pass Button */}
    {/* Circular FAB - Add Pass Button */}
<TouchableOpacity
  style={[styles.fabButton, { backgroundColor: THEME_COLORS.primaryAccent }]}
...
const getPassStyles = (currentTheme, nightMode) => StyleSheet.create({
  ...
  fabButton: {
    position: 'absolute',
    bottom: 90,              // 90px from bottom
    right: 30,               // 30px from right
    width: 56,               // Standard FAB size
    height: 56,              // Standard FAB size
    borderRadius: 28,        // Perfect circle (width/2)
    justifyContent: 'center',
  alignItems: 'center',
  shadowColor: '#000',
